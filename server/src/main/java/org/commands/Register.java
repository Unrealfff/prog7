...
public class Register extends Command {
     ...
     * @param console
     */
    public Register(Console console, FileManager fileManager){
    ...
    public Response execute(String[] arguments, String[] login) {
        try {
            ...
            if (fileManager.register(login)) {
                return new Response(1, "user registered");
            }
            return new Response(0, "could not register user");

        } catch (Exception e) {
            System.out.println("Error occurred while registering");
            return new Response(0, "could not register new user");
        }
    }
}